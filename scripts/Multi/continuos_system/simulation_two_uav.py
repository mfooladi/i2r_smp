...
class Robot:
	def __init__(self,id):
		...
		self.states = vector_field.init_field(self.geometry_name[self.agent_id - 1])[1]
		self.stage.append(vector_field.init_field(self.geometry_name[self.agent_id - 1])[0])
		self.G  = vector_field.init_field(self.geometry_name[self.agent_id - 1])[2]
		
		self.v_ref, self.islast, self.vertice1 = vector_field.vector_field(self.inital_position[0][0], self.inital_position[0][1],self.current_stage, self.states, self.G)
	...
	def flatoutput(self, pos, current_stage):
		...
		self.v1, self.islast, self.vertice1 = vector_field.vector_field(self.pos[0], self.pos[1], self.current_stage,
		                                                        self.states,
		                                                  self.G)
		self.v2, self.islast, self.vertice1 = vector_field.vector_field(self.pos2[0], self.pos2[1], self.current_stage, self.states, self.G)
		self.v3, self.islast, self.vertice1 = vector_field.vector_field(self.pos3[0], self.pos3[1], self.current_stage, self.states, self.G)
		self.v4, self.islast, self.vertice1 = vector_field.vector_field(self.pos4[0], self.pos4[1], self.current_stage, self.states, self.G)
		self.v5, self.islast , self.vertice1= vector_field.vector_field(self.pos5[0], self.pos5[1], self.current_stage, self.states, self.G)
		self.v6, self.islast , self.vertice1= vector_field.vector_field(self.pos6[0], self.pos6[1], self.current_stage, self.states, self.G)
		self.v7, self.islast , self.vertice1= vector_field.vector_field(self.pos7[0], self.pos7[1], self.current_stage, self.states, self.G)
		self.v8, self.islast , self.vertice1= vector_field.vector_field(self.pos8[0], self.pos8[1], self.current_stage, self.states, self.G)
		self.v9, self.islast , self.vertice1= vector_field.vector_field(self.pos9[0], self.pos9[1], self.current_stage, self.states, self.G)
		self.v10, self.islast , self.vertice1= vector_field.vector_field(self.pos10[0], self.pos10[1], self.current_stage, self.states, self.G)
	...
	def control_inputs(self,islast, v_ref, sigma2dot, sigma3dot, sigma4dot):
		...
		self.R_euler = [[np.cos(self.theta) * np.cos(self.psi), np.sin(self.phi) * np.sin(self.theta) * np.cos(self.psi) - np.cos(self.phi) *
		                ...
		                [np.cos(self.theta) * np.sin(self.psi), np.sin(self.phi) * np.sin(self.theta) * np.sin(self.psi) - np.cos(self.phi) * np.cos(self.psi),
	...
	def collision_check(self, p1, p2, v1, v2, agent_id):  # Velocity Obstacle method
		...
		g_set_1 = uConstraints.point_g_calculator(point_xy = p1, vertice1 = self.triangle_list[self.vertice1-1][0],
		                                          common_facet_points = [self.triangle_list[self.vertice1][1],self.triangle_list[self.vertice1][2]],vel_max = self.v_max)
		sides = len(g_set_1)
...
print ''
# initial_points = collision_avoid_get_init.init_coords_xy
# common_vertice = collision_avoid_get_init.common_facet_pose_xy
# tri = [initial_points[0], common_vertice[0], common_vertice[1]]
# tri_1 = [initial_points[0], common_vertice[0], common_vertice[1]]
# tri_2 = [initial_points[1], common_vertice[1], common_vertice[0]]
# positions = [np.array(initial_points[2]), np.array(initial_points[3])]
# uav_1 = {'id': 1, 'position_init': initial_points[2], 'trajectory': [initial_points[2]], 'triangle': tri_1}
# uav_2 = {'id': 2, 'position_init': initial_points[3], 'trajectory': [initial_points[3]], 'triangle': tri_2}
...
def updateSim(dt):
	...
	if robot1.finished == False:
		...
		if robot1.need_transition:
			...
			robot1.v_ref, robot1.islast, robot1.vertice1 = vector_field.vector_field(robot1.pos[0], robot1.pos[1],
			                                                          robot1.current_stage)
