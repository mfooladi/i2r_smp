...
def discrete_planner(geometry_name, init_position, goals_file = 'goals', goal_list = [], temp_obstacle = 0):
	...
	os.popen('triangle -Bnevga100. {}.poly'.format(parent_directory + '/geometry/' + geometry_name))  # -n is for producing the neighbor
...
class Robot(State):
	...
	def move(self, current_pos, current_vel, current_state = 1, current_patch = 1, dt = 0.05):
		...
		if  int(self.triangle_number(self.current_state)) in self.goal_list:
			...
			self.poly_pix.append(((poly_xy[0][0] - world_xmin) * world_scale_x, (poly_xy[0][1] - world_ymin) * world_scale_y, (poly_xy[1][0] - world_xmin) * world_scale_x,
			                    (poly_xy[1][1] - world_ymin) * world_scale_y,
			                    (poly_xy[2][0] - world_xmin) * world_scale_x, (poly_xy[2][1] - world_ymin) * world_scale_y))
			if self.agent_id == 1:
				visited_targets = canvas.create_polygon(self.poly_pix[-1], fill = '#020000', outline = 'black')
			if self.agent_id == 2:
				visited_targets = canvas.create_polygon(self.poly_pix[-1], fill = '#6600ff', outline = 'black')
...
pixleh = 768.
pixlew = 1366.
robot1 = Robot(r1_geometry, 4., 1, 5., r1_goals, r1_pos_init)
robot2 = Robot(r2_geometry, 4., 2, 5., r2_goals, r2_pos_init)
...
world_scale_y = 1966 / world_height
world_scale_x = 768 / world_width
world_scale_y = world_scale_x
print 'world scale', world_scale_x
win = Tk()
win.attributes("-topmost", True)
frame = Frame(win, width = 1966, height=768, bg = '#ffffff')
frame.pack()
...
win.geometry("{}x{}+{}+{}".format(1966, 768, 10, 100))
win.attributes("-topmost", True)
canvas = Canvas(frame, width = pixlew, height = pixleh, background = "#000000")
canvas.pack()
canvas.place(x = 500, y = 0)
image1 = Image.new("RGB", (int(pixleh), int(pixlew)))
